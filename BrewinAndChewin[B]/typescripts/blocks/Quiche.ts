...
export class Quiche {
    ...
    playerBreak(args: PlayerBreakBlockBeforeEvent) {
        ...
        if (!itemStack) return
        if (block.typeId != "brewinandchewin:quiche") return
    ...
    use(args: PlayerInteractWithBlockAfterEvent) {
        ...
        if ((!itemStack)||(!itemStack.hasTag("farmersdelight:is_knife"))) player.addEffect("speed",30*20)
        else dimension.spawnItem(new ItemStack("brewinandchewin:quiche_slice"),{x:location.x+0.5,y:location.y+0.5,z:location.z+0.5})
        const stage = block.permutation.getState("brewinandchewin:food_block_stage") as number
