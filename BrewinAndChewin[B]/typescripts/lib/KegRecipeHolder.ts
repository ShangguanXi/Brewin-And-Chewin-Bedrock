...
export class KegRecipeHolder {
    ...
    /**检查液体槽物品是否在液体列表里*/
    private isFluidTypeInFluidSlot(): boolean {
        const fluidId = this.container.getItem(5)?.typeId
        if (!fluidId) return false
        return fluidId in fluidType
    }
    /**获取 液体槽物品 对应的 物品id*/
    private getFluidContainerInFluidSlot(): string | undefined {
        const fluidId = this.container.getItem(5)?.typeId
        if (!fluidId) return undefined
        return fluidType[fluidId];
    }
    /** 检查容器槽物品是否在液体列表里*/
    private isFluidInContainerSlot(): boolean {
        const fluidInContainerSlot = this.container.getItem(4)?.typeId
        if (!fluidInContainerSlot) return false
        return fluidInContainerSlot in fluidType
    }
    /**  获取容器槽里的液体 对应的 物品id*/
    private getFluidContainerInContainerSlot(): string | undefined {
        const fluidId = this.container.getItem(4)?.typeId
        if (!fluidId) return undefined
        return fluidType[fluidId];
    }
    /**  检测温度*/
    public checkTemperature(): number {
        ...
        let coldBlock: number = 0;
        let hotBlock: number = 0;
        ...
        if (temperatureBlockNumber >= 5) return 1
        else if (temperatureBlockNumber >= 2 && temperatureBlockNumber <= 4) return 2
        else if (temperatureBlockNumber >= -1 && temperatureBlockNumber <= 1) return 3
        else if (temperatureBlockNumber >= -4 && temperatureBlockNumber <= -2) return 4
        else return 5
    }
    /**  配方匹配*/
    private matchesRecipe(recipe: Recipe): boolean {
        ...
        if (recipe.temperature !== temperature) return false;

        if (recipe.basefluid) {
            if (!fluidId || recipe.basefluid !== fluidId) return false;
            const hasEnoughFluid = (baseFluid.maxAmount >= 4 && baseFluid.amount >= 4) || (baseFluid.maxAmount < 4);
            if (!hasEnoughFluid) return false;
        }
    ...
    /**  检测输出槽是否可以输出配方*/
    private applyRecipe(recipe: Recipe): boolean {
        ...
        if (outputSlot.typeId !== recipeOutput) return false;
        const totalCount = outputSlot.amount + recipeOutputCount;
        const maxStack = outputSlot.maxAmount;
        if (totalCount > maxStack) return false;
        return true;
    }
    /**  配方进度*/
    public findMatchingRecipe(recipes: Recipe[]): Recipe | undefined {
        for (let recipe of recipes) {
            if (this.matchesRecipe(recipe) && this.applyRecipe(recipe)) {
                const progress = this.entity.getDynamicProperty("brewinandchewin:progress") as number ?? 0
                ...
                else this.entity.setDynamicProperty("brewinandchewin:progress", progress + 1);
    ...
    private calculatePercentageBar(numerator: number, denominator: number): string {
        if (denominator === 0) {
            return `0.00 / 100`;
        }
    ...
    public fillResultSlot(): void {
        ...
        if (!this.isFluidTypeInFluidSlot()) {
            ...
            if (resultCount == resultItem.maxAmount) return
        ...
        if (this.getFluidContainerInFluidSlot() != containerItem.typeId) return
        if (!resultItem) {
            if (!fluidItem) return
            this.container.setItem(6, new ItemStack(fluidItem.typeId))
    ...
    /**  手动往溶液槽装物品*/
    public fillFluidSlot(): void {
        if (!this.isFluidInContainerSlot()) return
        ...
        if (!fluidItem) {
            ...
            if (returnItem != resultItem.typeId) return
            const resultCount = resultItem.amount
            if (resultCount == resultItem.maxAmount) return
            resultItem.amount = resultCount + 1;
            this.container.setItem(5, new ItemStack(containerItem.typeId, 1))
        ...
        //溶液槽有物品时
        if (containerItem.typeId != fluidItem?.typeId) return

        ///输出槽无物品时
        if (!resultItem) {
            if (fluidItem.maxAmount >= 4) {
                if (fluidItem.amount > 3) return
