import { Block, Container, Entity, EntityComponentTypes, EntityInventoryComponent, GameMode, ItemComponentTypes, ItemDurabilityComponent, ItemStack, Player, Vector3, world } from "@minecraft/server";
...
export class ItemAPI {
     ...
     * @returns 
     */
    public static clear(containerType: Player|Entity, slot: number, number: number = 1) {

        const inventory = containerType.getComponent(EntityComponentTypes.Inventory) as EntityInventoryComponent;
        const container = inventory.container
        if (!container) return;
        const itemStack = container?.getItem(slot)
        if (!itemStack) return;
        if (containerType instanceof Player) {
            if (containerType.getGameMode() == GameMode.creative) return;
        }
        const itemAmount = itemStack.amount;
        if (itemAmount==1){
            container.setItem(slot, undefined);
