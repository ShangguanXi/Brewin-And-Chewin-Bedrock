...
const isId = (value) => typeof value === "string" && value.includes(":");
/** 校验外部发来的发酵桶配方，返回整理后的配方，不合法时返回原因 */
function parseKegRecipe(data) {
    if (!data || typeof data !== "object")
        return "message is not a JSON object";
    const { basefluid, fermentingtime, temperature, ingredients, result } = data;
    if (basefluid !== undefined && !isId(basefluid))
        return "'basefluid' must be an item id";
    if (!Number.isInteger(fermentingtime) || fermentingtime <= 0)
        return "'fermentingtime' must be a positive number of ticks";
    if (!Number.isInteger(temperature) || temperature < 1 || temperature > 5)
        return "'temperature' must be 1 ~ 5";
    if (!Array.isArray(ingredients) || ingredients.length < 1 || ingredients.length > 4)
        return "'ingredients' must have 1 ~ 4 entries";
    const parsed = [];
    for (const ingredient of ingredients) {
        if (isId(ingredient?.item))
            parsed.push({ item: ingredient.item });
        else if (isId(ingredient?.tag))
            parsed.push({ tag: ingredient.tag });
        else
            return "every ingredient needs an 'item' or a 'tag'";
    }
    if (!isId(result?.item))
        return "'result.item' must be an item id";
    const count = result.count ?? 1;
    if (!Number.isInteger(count) || count < 1)
        return "'result.count' must be a positive integer";
...
export class KegRecipeRegister {
     ...
     */
    register(args) {
        if (args.id !== "brewinandchewin:keg_recipe" && args.id !== "brewinandchewin:keg_fluid")
            return;
        let data;
        try {
        ...
        if (kegRecipes.some(existing => JSON.stringify(existing) === key))
            return;
