import { Block, Entity, EntityComponentTypes, GameMode, ItemStack, Player, world } from "@minecraft/server";
...
export class ItemAPI {
     ...
     * @returns
     */
    static clear(containerType, slot, number = 1) {
        const inventory = containerType.getComponent(EntityComponentTypes.Inventory);
        const container = inventory.container;
        if (!container)
            return;
        const itemStack = container?.getItem(slot);
        ...
        if (containerType instanceof Player) {
            if (containerType.getGameMode() == GameMode.creative)
                return;
        }
        const itemAmount = itemStack.amount;
        if (itemAmount == 1) {
